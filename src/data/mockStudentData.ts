...
export type NewStudentInput = Omit<Student, 'performance_level' | 'absence_group' | 'risk_level' | 'pass_status'>;

/**
 * Reusable helper that computes derived fields (performance_level, absence_group, risk_level, pass_status)
 * using the exact project classification logic.
 */
export function createStudentRecord(input: NewStudentInput): Student {
  const g1 = Number(input.G1);
  const g2 = Number(input.G2);
  const g3 = Number(input.G3);
  ...
  // 3. Risk Level (Using verified Athena ML Classification Rule)
  let risk_level: Student['risk_level'] = 'Moderate Risk';
  ...
  // 4. Pass Status
  const pass_status: Student['pass_status'] = g3 >= 10 ? 'Pass' : 'Fail';
  ...
  return {
    ...
    studytime: Number(input.studytime),
    famrel: Number(input.famrel),
    freetime: Number(input.freetime),
    goout: Number(input.goout),
    health: Number(input.health),
    performance_level,
    absence_group,
    risk_level,
    pass_status,
